...
#
# if [ "$ARCH" = "arm64" ] || [ "$ARCH" = "amd64" ]; then
#   log "正在获取 mihomo-meta 最新版本信息..."
#   log "当前设备架构: ${ARCH}"
#
#   MIHOMO_URL=$(curl -sL "https://api.github.com/repos/MetaCubeX/mihomo/releases/tags/Prerelease-Alpha" | grep "browser_download_url" | grep -oE "https://[^\"]*mihomo-linux-${ARCH}-alpha-[0-9a-z]+\.gz" | head -n 1)
#
#   if [ -n "$MIHOMO_URL" ]; then
#     log "目标下载地址: ${MIHOMO_URL}"
#     META_MAKEFILE=$(find ./ -path "*/mihomo-meta/Makefile" | head -n 1)
#     
#     if [ -f "$META_MAKEFILE" ]; then
#       sed -i \
#         -e '/^PKG_SOURCE/d' \
#         -e '/^PKG_MIRROR_HASH/d' \
#         -e '/^PKG_BUILD_/d' \
...
# 	curl -L -k -o \$(PKG_BUILD_DIR)/mihomo.gz "${MIHOMO_URL}"
...
# EOF
#       log "mihomo-meta Makefile 重写完成 ✔"
#     else
#       log "❌ 错误: 未找到 mihomo-meta Makefile"
#     fi
#
#     NIKKI_MAKEFILE=$(find ./ -path "*/nikki/Makefile" | head -n 1)
#     
#     if [ -f "$NIKKI_MAKEFILE" ]; then
#       sed -i \
#         -e '/^PKG_BUILD_DEPENDS:=golang\/host/d' \
#         -e '/^GO_PKG/d' \
#         -e '/golang-package.mk/d' \
#         -e 's/\$(GO_ARCH_DEPENDS) //g' \
#         -e '/GoBinPackage/d' \
#         "$NIKKI_MAKEFILE"
#       
#       sed -i '/\$(eval \$(call BuildPackage,nikki))/d' "$NIKKI_MAKEFILE"
#       echo "\$(eval \$(call BuildPackage,nikki))" >> "$NIKKI_MAKEFILE"
#
#       log "nikki Makefile Go 依赖清理完成 ✔"
#     fi
#
#   else
...
#   fi
# else
#   log "跳过 mihomo-meta 配置 (当前架构 $ARCH 不匹配预设规则)"
# fi
# cd "$PKG_PATCH"
...
if [ -d "./luci-theme-argon" ]; then
  ...
  cp -f "$GITHUB_WORKSPACE/images/bg1.jpg" "luci-theme-argon/htdocs/luci-static/argon/img/bg1.jpg" 2>/dev/null || true
